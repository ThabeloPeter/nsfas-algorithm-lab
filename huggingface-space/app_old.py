...
def calculate_sa_id_checksum(id_12_digits: str) -> int:
    ...
    Returns the expected checksum digit (0-9).
    """
    try:
...
def extract_sa_id_number(text: str) -> Optional[str]:
    """
    Extract and validate South African ID number (13 digits).
    ...
    for id_number in matches:
        ...
        if validate_sa_id_date(yy, mm, dd):
            ...
            continue  # Invalid date, skip this candidate
        ...
        if expected_checksum == checksum_digit:
            ...
            continue  # Invalid checksum, skip
...
def parse_smart_id_card(ocr_data: List[tuple], raw_text: str) -> Dict[str, Any]:
    ...
    if id_number:
        ...
        # Extract sex from ID number (digit 7: 0-4=Female, 5-9=Male)
        sex_digit = int(id_number[6])
        sex = 'M' if sex_digit >= 5 else 'F'
        result['sex'] = {'value': sex, 'confidence': 99, 'status': 'success'}
        ...
        logger.info(f"📅 Birth date extracted: {birth_date}")
